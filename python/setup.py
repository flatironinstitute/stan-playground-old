...
def npm_install_command(command_subclass):
    ...
    def modified_run(self):
        ...
        if os.name == 'nt':
            ...
            shell = False
        subprocess.check_call(['npm', 'install'], cwd=f'{this_directory}/stan_playground/js', shell=shell)
        subprocess.check_call(['npm', 'run', 'build'], cwd=f'{this_directory}/stan_playground/js', shell=shell)
        orig_run(self)
...
setup(
    name='stan_playground',
    version=__version__,
    author="Jeremy Magland",
    author_email="jmagland@flatironinstitute.org",
    url="https://github.com/scratchrealm/stan-playground",
    description="Create, run, and share Stan models in the browser",
